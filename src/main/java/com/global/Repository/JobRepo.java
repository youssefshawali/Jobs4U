...
//	   List<Job> findByCompany_Id(int companyId,String name);
	...
	@Query("SELECT job FROM Job job WHERE (job.jobTitle LIKE %?1% OR job.company.name LIKE %?1%) " +
		       "AND (?2 IS NULL OR job.experience = ?2) " +
		       "AND (?3 IS NULL OR job.category LIKE %?3%)")
		List<Job> findBySearchFilters(String title, Integer experience, String category);
	  
//	   @Query("SELECT job FRvOM Job job WHERE (job.jobTitle LIKE %?1% OR job.company.name LIKE %?1%)   AND job.workHours LIKE %?2% AND job.workPlaceType LIKE %?3% AND job.category LIKE %?4% ")
...
//		       "JOIN job.qualification qual " +
//		       "JOIN job.careerLevels career " +
		       "WHERE (job.jobTitle LIKE %?1% OR job.company.name LIKE %?1%) " +
		       "AND (?2 IS NULL OR job.workHours IN ?2) " +
		       "AND (?3 IS NULL OR job.workPlaceType IN ?3) "+
		       "AND (?4 IS NULL OR job.experience = ?4) " +
//		       "or (?5 IS NULL OR job.category LIKE %?5%) " +
		       "AND (?5 IS NULL OR skill.name IN ?5) " +
		       "AND (?7 IS NULL OR job.target IN ?6) " )
//		       "or (?8 IS NULL OR qual.degree IN ?7) " +
//		       "or (?9 IS NULL OR career.level IN ?8)")
