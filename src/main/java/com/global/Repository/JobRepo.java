...
public interface JobRepo extends JpaRepository<Job, Integer> {
	...
	@Query("SELECT job FROM Job job " + "LEFT JOIN job.skills skill " + "LEFT JOIN job.qualification qual "
			+ "LEFT JOIN job.careerLevels career " + "WHERE (job.jobTitle LIKE %?1% OR job.company.name LIKE %?1%) "
			...
			+ "AND (?8 IS NULL OR career.level IN ?8)")
	...
	List<Job> findByApplicantsId(int userId);
